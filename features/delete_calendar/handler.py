...
def delete_event_api(function_args: dict, timeZone:str = "Asia/Ho_Chi_Minh") -> dict:
    """
    ...
    if function_args.get('incorrect_datetime'):
        return invalid_time
    try:
        if function_args['datetime_ranges'][0]['start_datetime'] == function_args['datetime_ranges'][0]['end_datetime']:
            print("start_datetime == end_datetime")
            temp = datetime.strptime(function_args['datetime_ranges'][0]['end_datetime'], "%Y-%m-%d %H:%M:%S") + timedelta(seconds=1)
            function_args['datetime_ranges'][0]['end_datetime'] = temp.strftime("%Y-%m-%d %H:%M:%S")
