...
def get_freetime(busy_periods: list, time_min: str, time_max: str) -> list:
    ...
    for busy_period in busy_periods:
        ...
        if start > last_free_time:
            free_times.append({'start': last_free_time.isoformat(), 'end': start.isoformat()})
        last_free_time = end
    if last_free_time < dt_max:
        free_times.append({'start': last_free_time.isoformat(), 'end': dt_max.isoformat()})
    return free_times


def get_free_time_api(function_args: dict, timeZone:str = "Asia/Ho_Chi_Minh") -> dict:
