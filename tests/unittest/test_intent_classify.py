...
from bot.agent_extraction import extraction_model
...
test_extraction_cases = [
    ...
    ("Tạo lịch họp team vào lúc 14h chiều mai", (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")),
    ("Xoá lịch họp ngày mai", (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")),
...
def test_intent_classification(prompt, expected_intent):
    ...
    assert result["intent"] == expected_intent, f"Expected intent '{expected_intent}' but got '{result['intent']}' for prompt: {prompt}"
...
def test_datetime_extraction(prompt, expected_date):
    ...
    assert "datetime_ranges" in result, f"datetime_ranges key missing in result for prompt: {prompt}"
    assert len(result["datetime_ranges"]) > 0, f"No datetime ranges found for prompt: {prompt}"
    
    # Check if the extracted date matches the expected date
    extracted_date = result["datetime_ranges"][0]["start_datetime"].split()[0]  # Get just the date part
    assert extracted_date == expected_date, f"Expected date '{expected_date}' but got '{extracted_date}' for prompt: {prompt}"
...
def test_extraction_structure():
    ...
    assert isinstance(result["datetime_ranges"], list), "datetime_ranges should be a list"
