...
{list(calendar_features_map.keys())}
...
2. Xác định hành động chính người dùng muốn thực hiện (tạo, cập nhật, xóa, tìm kiếm sự kiện)
3. Chọn ý định chính xác từ danh sách đã cung cấp

## VÍ DỤ PHÂN LOẠI Ý ĐỊNH:
{intent_example_dict()}
""",
    "parameters": {
        "type": "object",
        "properties": {
            "intent": {
                "type": "string",
                "description": f"Ý định chính trong tin nhắn của người dùng. PHẢI thuộc một trong các giá trị sau: {list(calendar_features_map.keys())}"
            },
            "confidence": {
                "type": "number",
